...
if os.path.exists(configJsonPath):
    ...
    audiofilesPath = './media/audio/'
    configPath = './config/'
    audioJsonPath = '{}audio.json'.format(conf['configPath'])
    configJsonPath = '{}config.json'.format(conf['configPath'])
    ytdlConf = conf['ytdlFormatOptions']
    ...
    exampleConf = {'discordToken':'fdjkakjdfefehsabh93,.3mejnfe', 'commandPrefix':'?', 'ownerIds':[], 'audiofilesPath':'./media/audio/', 'configPath':'./config/' , 'ytdlFormatOptions': {'format': 'bestaudio/best','outtmpl': '%(extractor)s-%(id)s-%(title)s.%(ext)s', 'restrictfilenames': True, 'noplaylist': True, 'nocheckcertificate': True, 'ignoreerrors': False, 'logtostderr': False, 'quiet': True, 'no_warnings': True, 'default_search': 'auto', 'source_address': '0.0.0.0',"HighWaterMark":3145728}, 'ffmpeg_options':{'options': '-vn'},"ffmpeg_before_options":{"reconnect": 1, "reconnect_streamed": 1, "reconnect_delay_max":5}}
...
"""
if os.path.exists(configJsonPath):
    client.add_cog(Music(client, audioJsonPath, audiofilesPath, ytdlConf))
